...
main:
	...
	//Stores players health to memory
	LDR R1, address_of_pDamage
	STR R5, [R1]

	bl ranNum//Calls function ranNum so generate random number
	MOV R8, R1//For Displaying original value
calChal1:
	SUBS R1, R1, #2//Answer
        CMP R1, #1
		BGT calChal1//simulates do while loop
Challenge1:
	...
	//Takes in answer for Challege 1
	LDR R0, address_of_scan
	LDR R1, address_of_storeit
	bl scanf

	//Loads it to R0 for comparison
...
correct:
	...
	//Loads value of pDamage to R5 so that more damage can be added for winning
	LDR R5, address_of_pDamage
	LDR R5, [R5]

	ADD R5, R5, #3
	LDR R1, address_of_pDamage
	STR R5, [R1]		//Stores new pDamage value back to memory


	//Calls it again for display
