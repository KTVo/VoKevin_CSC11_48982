...
for_loop_boss:
	...
	//Prints the incr/total turn to fight boss
	...
	//Displays instruction to player
	LDR R0, address_of_bossInstr
	bl printf

	//Takes in input to attack BOSS
	...
	//Checks if input == 'a'
	CMP R1, R2
		SUBEQ R9, R9, R5 // only does damage if input == 'a'
		BNE DisplayMissed //misses if != 'a'

//Checks if boss is still alive and if turn is up
firstCheck:
	CMP R9, #0
		BLE checkWinBoss
...
//alerts player if they've missed
DisplayMissed:
	LDR R0, address_of_missMess
	bl printf
	bl firstCheck

//checks if person wins or lose
checkWinBoss:
	CMP R9, #0
		BLE bossWIN
		BGT bossLOSS

//Displays message if won
bossWIN:
